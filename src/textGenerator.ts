...
export class TextGenerator {
  ...
  public generate(options?: GeneratorOptions): string[] {
    ...
    do {
      const wordToStart =
        options?.wordToStart?.toLowerCase() ?? this.wordsToStart.getRandom();

      const result = this.generateInternal(
        wordToStart,
        minWordCount,
        maxWordCount,
        contextUsageDegree
  ...
  public generateSentence(options?: GeneratorOptions): string {
    ...
    if (generated?.length) {
      ...
      return (
        [firstWord[0].toUpperCase() + firstWord.substr(1)]
          .concat(rest)
          .join(" ") + "."
      );
    } else {
  ...
  private generateInternal(
    ...
    while (true) {
      const possibleNextWordAwareOfContext = preLastGeneratedWord
        ? this.wordStorage.get(
            TextGenerator.getKeyForMultipleWords(
              preLastGeneratedWord,
              lastGeneratedWord
            )
          )
        : null;
      ...
      const possibleNextWords =
        possibleNextWordAwareOfContext && Math.random() < contextAwarenessDegree
      ...
      if (
        resultWords.length > minWordCount &&
